...
add_action('astra_woo_shop_summary_wrap_bottom', function () {
	...
	if (!empty($categories)) {
		...
		echo '<div class="location"><a href="#" class="company-link">Zuora</a></div>';
		echo '<div class="type"><a href="' . $category->slug . '" class="role-link">' . $category->name . '</a></div>';
		echo '<div class="experience"><a href="' . $category->slug . '" class="role-link">' . $category->name . '</a></div>';
		echo '</div>';
	}
}, 20);
...
add_action('astra_woo_shop_after_summary_wrap', function () {
	global $product;
	echo '<div class="ast-shop-footer"><div class="salary">' . $product->get_regular_price() . '</div></div>';
}, 20);
